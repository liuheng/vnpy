...
def process_callback(line):
    ...
    create_task(cb_name, cb_args_type_list, cb_args_value_list, orignal_line)
    ...
    if line.count('*') == 1:
        ...
        if 'is_last' in line:
            on_line = 'virtual void on' + cb_name[2:] + '(dict data, dict error, int reqid, bool last) {};\n'
        else:
            on_line = 'virtual void on' + cb_name[2:] + '(dict data, dict error) {};\n'
    elif line.count('*') == 0:
...
def create_wrap(cb_name, line):
    ...
    if line.count('*') == 1:
        ...
        if "int" in line:
            on_line = 'virtual void on' + cb_name[2:] + '(dict data, dict error, int id, bool last)\n'
            override_line = '("on' + cb_name[2:] + '")(data, error, id, last);\n'
        else:
            on_line = 'virtual void on' + cb_name[2:] + '(dict data, dict error)\n'
            override_line = '("on' + cb_name[2:] + '")(data, error);\n'
    elif line.count('*') == 0:
        on_line = 'virtual void on' + cb_name[2:] + '()\n'
...
def create_process(cb_name, cb_args_type_list, cb_args_value_list):
    ...
    on_args_list = []
    print cb_name, cb_args_type_list
...
def process_function(line):
    ...
    # 生成.h文件中的主动函数部分
    if 'Req' in fc_name:
        req_line = 'int req' + fc_name[3:] + '(dict req, int reqid);\n'
        f_header_function.write(req_line)
        f_header_function.write('\n')


def create_function(fc_name, fc_args_type_list, fc_args_value_list):
    type_ = fc_args_type_list[0]
    struct = structDict[type_]

    fc_name = fc_name.strip()

    f_function.write('int TraderApi::' + fc_name[0].lower() + fc_name[1:] + '(dict req, int reqid)\n')
    f_function.write('{\n')
    ...
    f_function.write('    int i = this->api->' + fc_name + '(&myreq, reqid);\n')
...
f_cpp = open('xtp_trader_api.h', 'r')
f_task = open('xtp_td_task.cpp', 'w')
f_process = open('xtp_td_process.cpp', 'w')
f_function = open('xtp_td_function.cpp', 'w')
f_define = open('xtp_td_define.cpp', 'w')
f_switch = open('xtp_td_switch.cpp', 'w')
f_header_process = open('xtp_td_header_process.h', 'w')
f_header_on = open('xtp_td_header_on.h', 'w')
f_header_function = open('xtp_td_header_function.h', 'w')
f_wrap = open('xtp_td_wrap.cpp', 'w')
